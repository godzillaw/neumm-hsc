...
function tileScheme(avg: number | null): {
  ...
  if (avg === null) return { bg: '#F9FAFB', border: '#E5E7EB', text: '#9CA3AF', dot: '#D1D5DB', label: 'Untested' }
...
function overallColor(pct: number) {
  ...
  return '#185FA5'  // blue (building up)
}
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="flex flex-col items-center justify-center z-10">
        ...
        <span className="text-xs text-gray-400 font-medium mt-0.5">overall</span>
      </div>
    </div>
  )
}
...
function TopicTile({ topic }: { topic: TopicStat }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs font-semibold leading-snug" style={{ color: scheme.text }}>
        {topic.name}
      </p>
      <p className="text-xs mt-1 font-medium" style={{ color: scheme.dot }}>
        {scheme.label}
      </p>
    </div>
  )
  ...
  return (
    <div>
      ...
      {Object.entries(categories).map(([cat, catTopics]) => (
        <CategorySection key={cat} name={cat} topics={catTopics} />
      ))}
    </div>
  )
}
